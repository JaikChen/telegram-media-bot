...
# 群组管理命令：规则、关键词、页脚、白名单、转发等

from telegram import Update
from telegram.ext import ContextTypes
from db import *
from cleaner import clean_caption
from handlers.utils import is_admin, check_chat_permission
...
async def handle_addkw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) >= 3:
        ...
        await msg.reply_text(f"✅ 已添加关键词 `{args[2]}` 到频道 {chat_id}{' (regex)' if is_regex else ''}", parse_mode="Markdown")
    else:
...
async def handle_allowuser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) == 3:
        ...
        await msg.reply_text(f"✅ 已将用户 `{user_id}` 加入频道 {chat_id} 的白名单。", parse_mode="Markdown")
    else:
...
async def handle_blockuser(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) == 3:
        ...
        await msg.reply_text(f"🗑 已将用户 `{user_id}` 从频道 {chat_id} 的白名单中移除。", parse_mode="Markdown")
    else:
